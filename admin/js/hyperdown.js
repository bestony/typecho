...
(function() {
var Parser, slice = [].slice;

Parser = (function() {
  var array_keys, array_values, htmlspecialchars, pL, preg_quote, str_replace,
      trim, ucfirst;

  ucfirst = function(
      str) { return (str.charAt(0)).toUpperCase() + str.substring(1); };
  ...
  str_replace = function(search, replace, str) {
    ...
    if (search instanceof Array) {
      ...
      str =
          str.replace(new RegExp(search, 'g'), replace.replace(/\$/g, '$$$$'));
    }
    return str;
  };
  ...
  trim = function(str, ch) {
    ...
    if (ch != null) {
      search = '';
      for (i = j = 0, ref = ch.length - 1; 0 <= ref ? j <= ref : j >= ref;
           i = 0 <= ref ? ++j : --j) {
        c = ch[i];
        c = preg_quote(c);
        search += c;
  ...
  function Parser() {
    ...
    this.specialWhiteList = {table : 'table|tbody|thead|tfoot|tr|td|th'};
    this.hooks = {};
    this.html = false;
    this.line = false;
    this.blockParsers = [
      [ 'code', 10 ], [ 'shtml', 20 ], [ 'pre', 30 ], [ 'ahtml', 40 ],
      [ 'shr', 50 ], [ 'list', 60 ], [ 'math', 70 ], [ 'html', 80 ],
      [ 'footnote', 90 ], [ 'definition', 100 ], [ 'quote', 110 ],
      [ 'table', 120 ], [ 'sh', 130 ], [ 'mh', 140 ], [ 'dhr', 150 ],
      [ 'default', 9999 ]
    ];
    this.parsers = {};
  }
  ...
  Parser.prototype.makeHtml = function(text) {
    ...
    this.definitions = {};
    this.holders = {};
    this.uniqid = (Math.ceil(Math.random() * 10000000)) +
  ...
  Parser.prototype.enableHtml = function(
      html1) { this.html = html1 != null ? html1 : true; };

  Parser.prototype.enableLine = function(
      line1) { this.line = line1 != null ? line1 : true; };
    ...
    if (this.footnotes.length > 0) {
      ...
      while (val = this.footnotes.shift()) {
        if (typeof val === 'string') {
          val += " <a href=\"#fnref-" + index +
                 "\" class=\"footnote-backref\">&#8617;</a>";
        } else {
          val[val.length - 1] += " <a href=\"#fnref-" + index +
                                 "\" class=\"footnote-backref\">&#8617;</a>";
  ...
  Parser.prototype.parse = function(text, inline, offset) {
    var block, blocks, end, extract, html, j, len, lines, method, result, start,
        type, value;
    if (inline == null) {
      inline = false;
    }
  ...
  Parser.prototype.releaseHolder = function(text, clearHolders) {
    ...
    while ((text.indexOf("\r")) >= 0 && deep < 10) {
      text = str_replace(array_keys(this.holders), array_values(this.holders),
                         text);
      deep += 1;
    }
    if (clearHolders) {
  ...
  Parser.prototype.optimizeLines = function(html) {
    ...
    last = 0;
    regex = new RegExp(
        "class=\"line\" data\\-start=\"([0-9]+)\" data\\-end=\"([0-9]+)\" (data\\-id=\"" +
            this.uniqid + "\")",
        'g');
    if (this.line) {
      return html.replace(regex, function() {
        ...
        if (last !== parseInt(matches[1])) {
          replace = 'class="line" data-start="' + last +
                    '" data-start-original="' + matches[1] + '" data-end="' +
                    matches[2] + '" ' + matches[3];
        } else {
          replace = matches[0];
        }
  ...
  Parser.prototype.parseInline = function(text, whiteList, clearHolders,
    ...
    text = text.replace(
        /(^|[^\\])(`+)(.+?)\2/mg, (function(_this) {
          return function() {
            ...
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return matches[1] +
                   _this.makeHolder('<code>' + (htmlspecialchars(matches[3])) +
                                    '</code>');
          };
        })(this));
    text = text.replace(
    ...
    text = text.replace(
        /<(\/?)([a-z0-9-]+)(\s+[^>]*)?>/ig, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            if (_this.html ||
                (('|' + _this.commonWhiteList + '|' + whiteList + '|')
                     .indexOf('|' + matches[2].toLowerCase() + '|')) >= 0) {
    ...
    text = text.replace(
        ...
        (function(_this) {
          return function() {
            ...
            return _this.makeHolder("<img src=\"" + url + "\" alt=\"" +
        ...
        (function(_this) {
          return function() {
            ...
            result = _this.definitions[matches[2]] != null
                         ? "<img src=\"" + _this.definitions[matches[2]] +
                               "\" alt=\"" + escaped + "\" title=\"" + escaped +
                               "\">"
                         : escaped;
    ...
    text = text.replace(
        ...
        (function(_this) {
          return function() {
            var escaped, matches, url;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            escaped = _this.parseInline(_this.escapeBracket(matches[1]), '',
                                        false, false);
            url = _this.escapeBracket(matches[2]);
            url = _this.cleanUrl(url);
            return _this.makeHolder("<a href=\"" + url + "\">" + escaped +
                                    "</a>");
          };
        })(this));
    text = text.replace(
        ...
        (function(_this) {
          return function() {
            ...
            escaped = _this.parseInline(_this.escapeBracket(matches[1]), '',
                                        false, false);
            result = _this.definitions[matches[2]] != null
                         ? "<a href=\"" + _this.definitions[matches[2]] +
                               "\">" + escaped + "</a>"
                         : escaped;
            return _this.makeHolder(result);
          };
    ...
    text = text.replace(/<([_a-z0-9-\.\+]+@[^@]+\.[a-z]{2,})>/ig,
                        '<a href="mailto:$1">$1</a>');
    if (enableAutoLink) {
      regex = new RegExp("(^|[^\"])((https?):[" + pL +
                             "_0-9-\\./%#!@\\?\\+=~\\|\\,&\\(\\)]+)($|[^\"])",
                         'ig');
      text = text.replace(regex, (function(_this) {
                            return function() {
                              ...
                              return matches[1] + "<a href=\"" + matches[2] +
                                     "\">" + link + "</a>" + matches[4];
                            };
                          })(this));
    }
  ...
  Parser.prototype.parseInlineCallback = function(text) {
    text = text.replace(
        /(\*{3})((?:.|\r)+?)\1/mg, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return '<strong><em>' + (_this.parseInlineCallback(matches[2])) +
                   '</em></strong>';
          };
        })(this));
    text = text.replace(
        /(\*{2})((?:.|\r)+?)\1/mg, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return '<strong>' + (_this.parseInlineCallback(matches[2])) +
                   '</strong>';
          };
        })(this));
    text = text.replace(
        /(\*)((?:.|\r)+?)\1/mg, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    ...
    text = text.replace(
        /(\s+|^)(_{3})((?:.|\r)+?)\2(\s+|$)/mg, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return matches[1] + '<strong><em>' +
                   (_this.parseInlineCallback(matches[3])) + '</em></strong>' +
                   matches[4];
          };
        })(this));
    text = text.replace(
        /(\s+|^)(_{2})((?:.|\r)+?)\2(\s+|$)/mg, (function(_this) {
          return function() {
            var matches;
            matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
            return matches[1] + '<strong>' +
                   (_this.parseInlineCallback(matches[3])) + '</strong>' +
                   matches[4];
          };
        })(this));
  ...
  Parser.prototype.parseBlockList = function(block, key, line, state) {
    ...
    if (!!(matches = line.match(/^(\s*)((?:[0-9]+\.)|\-|\+|\*)\s+/i))) {
      ...
      return false;
               ...
               !line.match(/^\s*\[((?:[^\]]|\\\]|\\\[)+?)\]:\s*(.+)$/)) {
      if ((state.empty <= 1) && !!(matches = line.match(/^(\s+)/)) &&
          matches[1].length > block[3]) {
        state.empty = 0;
        this.setBlock(key);
        return false;
  ...
  Parser.prototype.parseBlockAhtml = function(block, key, line, state) {
    ...
    if (this.html) {
      htmlTagRegExp =
          new RegExp("^\\s*<(" + this.blockHtmlTags + ")(\\s+[^>]*)?>", 'i');
      if (matches = line.match(htmlTagRegExp)) {
        if (this.isBlock('ahtml')) {
          ...
          htmlTagAllRegExp = new RegExp(
              "\\s*<(" + this.blockHtmlTags + ")(\\s+[^>]*)?>", 'ig');
          while (true) {
  ...
  Parser.prototype.parseBlockHtml = function(block, key, line, state) {
    var matches, tag;
    if (!!(matches = line.match(new RegExp(
               "^\\s*<(" + state.special + ")(\\s+[^>]*)?>", 'i')))) {
  ...
  Parser.prototype.parseBlockMh = function(block, key, line, state, lines) {
    var matches;
    if (!!(matches = line.match(/^\s*((=|-){2,})\s*$/)) &&
        ((block != null) && block[0] === 'normal' &&
         !lines[block[2]].match(/^\s*$/))) {
  ...
  Parser.prototype.optimizeBlocks = function(_blocks, _lines) {
    var block, blocks, from, isEmpty, key, lines, moved, nextBlock, prevBlock,
        to, type, types;
    ...
    while (blocks[key] != null) {
      ...
      if ('pre' === type) {
        isEmpty =
            (lines.slice(block[1], block[2] + 1))
                .reduce(function(
                            result,
                            line) { return (line.match(/^\s*$/)) && result; },
                        true);
        if (isEmpty) {
          block[0] = type = 'normal';
        }
  ...
  Parser.prototype.parsePre = function(lines, value, start) {
    ...
    lines = lines.map(function(
        line) { return htmlspecialchars(line.substring(4)); });
  ...
  Parser.prototype.parseAhtml = function(
      lines, value,
      start) { return trim((this.markLines(lines, start)).join("\n")); };
  ...
  Parser.prototype.parseMath = function(lines, value, start, end) {
    return '<p>' + (this.markLine(start, end)) +
  ...
  Parser.prototype.parseMh = function(
      lines, num, start, end) { return this.parseSh(lines, num, start, end); };
  ...
  Parser.prototype.parseList = function(lines, value, start) {
    var found, html, j, key, l, lastType, leftLines, leftStart, len, len1, line,
        matches, minSpace, row, rows, secondFound, secondMinSpace, space, text,
        type;
    html = '';
    minSpace = 99999;
  ...
  Parser.prototype.parseTable = function(lines, value, start) {
    var aligns, body, column, columns, head, html, ignores, j, key, l, last,
        len, len1, line, num, output, row, rows, tag, text;
    ignores = value[0], aligns = value[1];
    head = ignores.length > 0 &&
           (ignores.reduce(function(prev, curr) { return curr + prev; })) > 0;
    html = '<table>';
    body = head ? null : true;
    output = false;
    for (key = j = 0, len = lines.length; j < len; key = ++j) {
      ...
      for (l = 0, len1 = rows.length; l < len1; l++) {
        ...
        if (row.length > 0) {
          last += 1;
          columns[last] =
              [ (columns[last] != null ? columns[last][0] + 1 : 1), row ];
      ...
      html += '<tr';
      if (this.line) {
        html += ' class="line" data-start="' + (start + key) + '" data-end="' +
                (start + key) + '" data-id="' + this.uniqid + '"';
      }
      html += '>';
      for (key in columns) {
        column = columns[key];
  ...
  Parser.prototype.parseDefinition = function() { return ''; };

  Parser.prototype.parseHtml = function(lines, type, start) {
    lines = lines.map((function(_this) {
      return function(line) {
        return _this.parseInline(line, _this.specialWhiteList[type] != null
  ...
  Parser.prototype.cleanUrl = function(url) {
    ...
    regexUrl = new RegExp("^\\s*((http|https|ftp|mailto):[" + pL +
                              "_a-z0-9-:\\.\\*/%#!@\\?\\+=~\\|\\,&\\(\\)]+)",
                          'i');
    regexWord = new RegExp(
        "^\\s*([" + pL + "_a-z0-9-:\\.\\*/%#!@\\?\\+=~\\|\\,&]+)", 'i');
