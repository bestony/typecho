    ...
    function scroll(inputLine) {
        ...
        if (typeof inputLine != "undefined") {
            for (var a = 0; a < previewRows.length; a++) {
                ...
                if (inputLine >= item[0] && inputLine <= item[1]) {
                    if (
                        a == previewRows.length - 1 ||
                        item[2] < previewScrollTop ||
                        previewRows[a + 1][2] >
                            previewScrollTop + preview.height()
                    ) {
        ...
        for (var j = 0; j < previewRows.length; j++) {
            ...
            if (current[0] >= item[0] && current[1] <= item[1]) {
                var nextPos = previewRows[j + 1]
                    ? previewRows[j + 1][2]
                    : preview.get(0).scrollHeight;
                scrollPos =
                    (i == 0 ? 0 : item[2]) + (nextPos - item[2]) * percent;
...
(function($) {
    ...
    $.fn.dropdownMenu = function(options) {
        this.each(function() {
            var menu = this,
                s = $.extend({ menuEl: null, btnEl: null }, options);
        ...
        build: function(options) {
            ...
            this.each(function() {
                ...
                this.tableDnDConfig = {
                    ...
                    onDragClass: options.onDragClass
                        ? options.onDragClass
                        : "tDnD_whileDrag",
                ...
                if (
                    ...
                    var h = $("thead", this),
                        count = $("th", h).length,
                        f = $(
                            '<tfoot><tr><td style="padding:0;height:0;line-height:0;border:none" colspan="' +
                                count +
                                '"></td></tr></tfoot>'
                        ).insertAfter(h),
                        l = $("tr:last", this);
                    ...
                    jQuery(rows[i])
                        .mousedown(function(ev) {
                            if (ev.target.tagName == "TD") {
                                ...
                                jQuery.tableDnD.mouseOffset = jQuery.tableDnD.getMouseOffset(
                                    this,
                                    ev
                                );
        ...
        */
        ...
        mousemove: function(ev) {
            ...
            if (mousePos.y - yOffset < config.scrollAmount) {
                ...
                if (
                    windowHeight - (mousePos.y - yOffset) <
                    config.scrollAmount
                ) {
                    window.scrollBy(0, config.scrollAmount);
                }
