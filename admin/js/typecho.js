...
w.Typecho = {
  ...
  editorResize : function(id, url) {
    $('#' + id).resizeable({
      minHeight : 100,
      afterResize : function(h) { $.post(url, {size : h}); }
    })
  },
...
function scrollableEditor(el, preview) {
  var styles = el.css(), lastWidth = el.width(), lastFocus = null, merge = [],
      rows = [], previewRows = [], css = {
        display : 'block',
        'position' : 'absolute',
        'left' : '-99999px',
        'top' : '-99999px'
      },
      test = $('<div></div>').appendTo(document.body), focused = false;
  ...
  function reload(input) {
    var text = el.val(), lines = text.split("\n"), h = 0;
  ...
  function scroll(inputLine) {
    var height = el.height(), offset = (el.innerHeight() - height) / 2,
        scrollTop = el.scrollTop() - offset,
        previewScrollTop = preview.scrollTop(), percent = 0, scrollPos = 0,
        current = null;
    ...
    for (var i = 0; i < merge.length; i++) {
      ...
      if (scrollTop <= current[2]) {
        percent =
            (scrollTop - current[3]) * current[4] / (current[2] - current[3])
        break;
      }
    }
    ...
    for (var j = 0; j < previewRows.length; j++) {
      ...
      if (current[0] >= item[0] && current[1] <= item[1]) {
        var nextPos = previewRows[j + 1] ? previewRows[j + 1][2]
                                         : preview.get(0).scrollHeight;
  ...
  function scrollPreview() {
    var height = el.height(), offset = (el.innerHeight() - height) / 2,
        previewScrollTop = preview.scrollTop(), found = false, current;
    ...
    current = found ? previewRows[i > 0 ? i - 1 : 0]
                    : previewRows[previewRows.length - 1];

    var start = current[0] > 0 ? rows[current[0] - 1] : 0,
        end = rows[current[1]],
        nextPos =
            found ? previewRows[i > 0 ? i : 1][2] : preview.get(0).scrollHeight,
  ...
  function reloadPreview(input) {
    ...
    $('.line', preview).each(function() {
      var t = $(this), start = t.data('start'), end = t.data('end'),
          startOriginal = t.data('start-original'),
      ...
      if (typeof startOriginal != 'undefined') {
        merge.push(
            [ start, startOriginal - 1, rows[startOriginal - 1], last, 0 ]);
        merge.push(
            [ startOriginal, end, rows[end], rows[startOriginal - 1], 1 ]);
      } else {
  ...
  el.on('focus', function() { focused = true; }).on('blur', function() {
    focused = false;
    getFoucsElement(lastFocus).removeClass('focus');
  });
  ...
  el.on('DOMMouseScroll mousewheel touchmove', function() { scroll(); });

  preview.on('DOMMouseScroll mousewheel touchmove',
             function() { scrollPreview(); });
...
$.fn.dropdownMenu = function(options) {
  this.each(function() {
    var menu = this, s = $.extend({menuEl : null, btnEl : null}, options);
...
$.fn.resizeable = function(options) {
  var s = $.extend({minHeight : 100, afterResize : null}, options);
...
$.fn.tableSelectable = function(options) {
  ...
  $(s.rowEl, this)
      .each(function() {
        $(s.checkEl, this)
            .click(function(e) { clickRow($(this).parents(s.rowEl)); });
      })
      .click(function(e) {
        var target = $(e.toElement ? e.toElement : e.target),
            tagName = target.prop('tagName').toLowerCase();

        if ($.inArray(tagName, [ 'input', 'textarea', 'a', 'button', 'i' ]) >=
                0 &&
 ...
 * limitations to the styles that can be associated with a row (such as you
 ...
 * function takes 2 parameters: the table and the row which the user has started
 * to drag. onAllowDrop Pass a function that will be called as a row is over
 * another row. If the function returns true, allow dropping on that row,
 * otherwise not. The function takes 2 parameters: the dragged row and the row
 * under the cursor. It returns a boolean: true allows the drop, false doesn't
 * allow it. scrollAmount This is the number of pixels to scroll if the user
 * moves the mouse cursor to the top or bottom of the window. The page should
 * automatically scroll up or down as appropriate (tested in IE6, IE7, Safari,
 * FF2, FF3 beta)
 *
 ...
 *
 * Known problems:
 * - Auto-scoll has some problems with IE7  (it scrolls even when it shouldn't),
 * work-around: set scrollAmount to 0
 *
 * Version 0.2: 2008-02-20 First public version
 * Version 0.3: 2008-02-07 Added onDragStart option
 *                         Made the scroll amount configurable (default is 5 as
...
jQuery.tableDnD =
    {
        ...
        this.each(function() {
          ...
          if (0 == $('tfoot', this).length && 0 < $('thead', this).length) {
            var h = $('thead', this), count = $('th', h).length,
                f = $('<tfoot><tr><td style="padding:0;height:0;line-height:0;border:none" colspan="' +
                      count + '"></td></tr></tfoot>')
                        .insertAfter(h),
                l = $('tr:last', this);
      ...
      makeDraggable : function(table) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          // and Header rows) inspired by John Tarr and Famic
      ...
      /**
         ...
         that element. To do this we need the element's position and the mouse
         position
       */
      ...
      /**
         Get the position of an element by going up the DOM tree and adding up
         all the offsets
       */
      getPosition : function(e) {
        ...
        if (e.offsetHeight == 0) {
          ...
          the solution is likewise noted there, grab the offset of a table cell
          in the row - the firstChild. note that firefox will return a text node
          as a first child, so designing a more thorough solution may need to
          take that into account, for now this seems to work in firefox, safari,
          ie
        */
      ...
      mousemove : function(ev) {
        ...
        if (y != jQuery.tableDnD.oldY) {
          ...
          // If we're over a row then move the dragged row to there so that the
          // user sees the effect dynamically
      ...
      /**
         We're only worried about the y position really, because we can only
      ...
      mouseup : function(e) {
        if (jQuery.tableDnD.currentTable && jQuery.tableDnD.dragObject) {
          ...
          // The row will already have been moved to the right place so we just
          // reset stuff
          if (config.onDragClass) {
...
var pasteEventName = getPasteEvent() + ".mask", ua = navigator.userAgent,
    iPhone = /iphone/i.test(ua), android = /android/i.test(ua), caretTimeoutId;
 ...
 *http://flesler.blogspot.com Dual licensed under MIT and GPL. Date: 4/09/2012
 *
 ...
 * @param {String, Number, DOMElement, jQuery, Object} target Where to scroll
 *the matched elements. The different options for target are:
 *		- A number position (will be applied to all axes).
 *		- A string position ('44', '100px', '+=90', etc ) will be applied
 *to all axes
 *		- A jQuery/DOM element ( logically, child of the element to scroll
 *)
 *		- A string selector, that will be relative to the element to
 *scroll ( 'li:eq(2)', etc )
 *		- A hash { top:x, left:y }, x and y can be any kind of
 *number/string like above.
 *		- A percentage of the container's dimension/s, for example: 50% to
 *go to the middle.
 *		- The string 'max' for go-to-end.
 * @param {Number, Function} duration The OVERALL length of the animation, this
 *argument can be the settings object instead.
 * @param {Object,Function} settings Optional set of settings or the onAfter
 *callback.
 *	 @option {String} axis Which axis must be scrolled, use 'x', 'y', 'xy'
 *or 'yx'.
 *	 @option {Number, Function} duration The OVERALL length of the
 *animation.
 *	 @option {String} easing The easing method for the animation.
 *	 @option {Boolean} margin If true, the margin of the target element will
 *be deducted from the final position.
 *	 @option {Object, Number} offset Add/deduct from the end position. One
 *number for both axes or { top:x, left:y }.
 *	 @option {Object, Number} over Add/deduct the height/width multiplied by
 *'over', can be { top:x, left:y } when using both axes.
 *	 @option {Boolean} queue If true, and both axis are given, the 2nd axis
 *will only be animated after the first one ends.
 *	 @option {Function} onAfter Function to be called after the scrolling
 *ends.
 *	 @option {Function} onAfterFirst If queuing is activated, this function
 *will be called after the first scrolling ends.
 ...
 *			$('#container').scrollTo( second_child, { duration:500,
 *axis:'x', onAfter:function(){ alert('scrolled!!');
 *			}});
 *
...
var $scrollTo = $.scrollTo = function(
    target, duration,
    settings) { $(window).scrollTo(target, duration, settings); };
  ...
  return this._scrollable()
      .each(function() {
        ...
        $.each(settings.axis.split(''), function(i, axis) {
          var Pos = axis == 'x' ? 'Left' : 'Top', pos = Pos.toLowerCase(),
              key = 'scroll' + Pos, old = elem[key],
        ...
        function animate(callback) {
          $elem.animate(
              attr, duration, settings.easing,
              callback &&
                  function() { callback.call(this, target, settings); });
        };
      })
      .end();
};
...
$scrollTo.max = function(elem, axis) {
  ...
  var size = 'client' + Dim, html = elem.ownerDocument.documentElement,
